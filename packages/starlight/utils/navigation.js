...
function linkFromSidebarLinkItem(item, locale) {
	...
	const label = pickLang(item.translations, localeToLang(locale)) || item.label;
	return makeSidebarLink(href, label, undefined, item.attrs);
}
...
function linkFromInternalSidebarLinkItem(item, locale) {
	...
	return makeSidebarLink(slugToPathname(route.slug), label, undefined, attrs);
}
...
/** Create a link entry */
function makeLink({ attrs = {}, badge = undefined, ...opts }) {
 ...
 */
const intermediateSidebars = new Map();

/** Get the sidebar for the current page using the global config. */
export function getSidebar(pathname, locale) {
...
export function getSidebarFromConfig(sidebarConfig, pathname, locale) {
...
export function getSidebarHash(sidebar) {
	let hash = 0;
...
export function getPrevNextLinks(sidebar, paginationEnabled, config) {
